...
\begin{frame}
  \begin{block}{Scientific programming/software}
    ...
    \end{itemize}
    See Gentleman et al. (2004) Genome Biology (Bioconductor paper XXX
CITE) for an example of successful scientific software. 
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \item Comprehensive \R Archive Network (CRAN) $\sim$ 11539 packages
    [2017-10-03] (cf 1000 in April 2007).  XXX
...
\item Jupyter (https://irkernel.github.io/)
    \item \ldots
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  (Mostly seen when reading in data frames XXX)
...
% XXX show example of memory savings...
...
(Humour): \url{http://simplystatistics.org/2013/01/31/paste0-is-statistical-computings-most-influential-contribution-of-the-21st-century/}
...
\begin{frame}
  ...
  \item ?dget / ?dput --- read/write ASCII representation of an \R
    object.
  \item XXX Package readxl for reading Excel files.
  \end{itemize}
  \ds
...
\begin{frame}
  \frametitle{Example: HOTPO rule XXX}
\end{frame}
...
\url{https://www.rstudio.com/products/rstudio/release-notes/debugging-with-rstudio/}
...
\url{http://mirrors.ebi.ac.uk/CRAN/}. \ds % XXX this is now dead.
