...
%% $Id: rpc.tex,v 1.45 2012/01/03 15:50:36 stephen Exp $
...
\begin{frame}
  ...
  \item 22 June 2012: release 2.15.1 "Roasted Marshmallows".  
    Stable, multi-platform. 
    (2.15.2 "Trick or Treat" schedules for 26 October 2012; 
    2.16.0 due March/April 2013.)
  \item R-core now 20 people, key academics in field, 
...
\begin{frame}
  ...
  \item Comprehensive \R Archive Network (CRAN) $\sim$ 3318 packages
    [2011-09-30] (cf 1000 in April 2007).
...
\begin{frame}[fragile]
  ...
  \item \Rfunction{round()}, \Rfunction{ceil()}, \Rfunction{floor()}, \Rfunction{signif()}
...
%% + 
%% \end{rc}
<<wrong, eval=FALSE,highlight=FALSE>>=
x <- (1 + (2 * 3)
+
@ 
...
\begin{frame}
  ...
  \item Use a good editor that helps you spot mistakes (e.g. paren
    matching, syntax highlighting).  Examples: Emacs/ESS, gedit, Rstudio. 
...
\begin{frame}[fragile]
  ...
  \item \Rfunction{dim}, \Rfunction{row}, \Rfunction{ncol}
...
\begin{frame}[fragile]
  ...
  (e.g. \Rfunction{l\$height} or \Rfunction{l[['heigth']]} -- if named list) %% $
...
\begin{frame}
  ...
  \texttt{S4} and \texttt{S4 ReferenceClasses}. \ds
...
\begin{frame}
  ...
  \item \Rpackage{ggplot2} -- \url{http://had.co.nz/ggplot2/}
  \end{itemize}
\end{frame}
...
\item \R graphics gallery \\
  ...
  \url{http://rgm2.lab.nig.ac.jp/RGM2/images.php}
...
summary(s1)
h <- scan('http://goo.gl/go4Ze') ## heights.dat
summary(h)
@
...
\begin{frame}[fragile]
  \frametitle{The \Robject{...} arguments}
<<plt, out.width = '.4\\linewidth', fig.show = 'hold', size='tiny', fig.width = 6, fig.height = 4>>=
p <- function(x, y, ...) {
...
p(rnorm(10, 0, 1), rnorm(10, 0, 5), main = "Expect swap", col = "red")
...
\url{http://www.stats.bris.ac.uk/R}. \ds
...
your \texttt{.bashrc} file (e.g. PWF linux)
\begin{verbatim}
