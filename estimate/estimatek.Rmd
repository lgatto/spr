---
title: "Example knitr document: estimating π"
...
Not everything is yet working (October 2016) in this document,
compared to the Rnw version.  The bookdown package might be more
useful for generating cross-references to figure and table captions.
...
<!-- Within RStudio, there is a handy ompile PDF  button. -->

knitr is newer than Sweave, and is more flexible.  Both estimatek.Rmd
and estimatek.pdf are available from: `https://github.com/lgatto/spr/tree/master/estimate`

(you will need `estimatek.Rmd` and `diff.R`)
...
Or you can add a caption:
<!-- Table~\ref{tab:example}. -->

```{r}
kable(df, caption="Example output from xtable.",label="tab:example")
```
...
```{r external-code,chunk=FALSE,include=FALSE}
